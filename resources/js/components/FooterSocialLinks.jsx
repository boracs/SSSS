...
function SocialIcon({ id }) {
    switch (id) {
        ...
        case "tiktok":
            return (
                <svg className="h-4 w-4" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
                    <path d="M12.525.02c1.31-.02 2.61-.01 3.91-.02.08 1.53.63 3.09 1.75 4.17 1.12 1.11 2.7 1.62 4.24 1.79v4.03c-1.44-.05-2.89-.35-4.2-.97-.57-.26-1.1-.59-1.62-.93-.01 2.92.01 5.84-.02 8.75-.08 1.4-.54 2.79-1.35 3.94-1.31 1.92-3.58 3.17-5.91 3.21-1.43.08-2.86-.31-4.08-1.03-2.02-1.19-3.44-3.37-3.65-5.71-.02-.5-.03-1-.01-1.49.18-1.9 1.12-3.72 2.58-4.96 1.66-1.44 3.98-2.13 6.15-1.72.02 1.48-.04 2.96-.04 4.44-.99-.32-2.15-.11-2.96.74-.75.88-.97 2.13-.5 3.22.46 1.01 1.45 1.74 2.54 1.85 1.02.1 2.06-.22 2.83-.88.77-.66 1.19-1.62 1.22-2.63.03-1.5.01-3.01.02-4.51.01-2.68-.01-5.36.02-8.04z" />
                </svg>
            );
        default:
            return null;
...
/** Iconos de redes compartidos vía Inertia (`socialLinks` desde config). */
export default function FooterSocialLinks({ className = "" }) {
    ...
    return (
        <div className={`flex flex-wrap items-center gap-2 ${className}`}>
            {links.map((link) => (
                <SocialButton key={link.id} href={link.href} label={link.label}>
