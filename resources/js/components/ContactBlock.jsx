...
export default function ContactBlock({
    ...
    const panelId = `contact-panel-${String(fallbackName ?? "contacto")
        .toLowerCase()
        .replace(/\s+/g, "-")}`;
    ...
    return (
        <>
            <button
                type="button"
                onClick={onToggle}
                className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-full bg-amber-500 px-5 py-2.5 text-sm font-bold text-slate-950 shadow-md transition hover:bg-amber-400"
                aria-expanded={open}
            ...
            {open ? (
                <div id={panelId} className="mt-4 space-y-3 rounded-xl border border-amber-500/20 bg-slate-950/40 p-4">
                    {contact?.phone && contact?.phoneTel ? (
                        ...
                        >
                            <Phone className="h-5 w-5 shrink-0 text-amber-300" aria-hidden />
                            <div>
                    ...
                    {contact?.email ? (
                        <a
                            ...
                            className="flex items-center gap-3 rounded-xl border border-white/10 bg-white/5 px-4 py-3 transition hover:bg-white/10"
                        >
                            <Mail className={`h-5 w-5 shrink-0 ${mailIconClassName}`} aria-hidden />
                            <div>
                                <p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">
                                    Correo
                                </p>
