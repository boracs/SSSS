...
export default function AdminAuctionsEdit({ auction, statuses = [], categories = [] }) {
    ...
    const [data, setData] = React.useState({
        ...
        category: auction.category || "surfboard",
        starting_price: String(auction.starting_price_eur ?? ""),
        min_increment: String(auction.min_increment_eur ?? "5"),
        reserve_price: auction.reserve_price_eur != null ? String(auction.reserve_price_eur) : "",
    ...
    const submit = (e) => {
        ...
        setProcessing(true);
        router.post(route("admin.auctions.update", auction.id), fd, {
            forceFormData: true,
        ...
        <Layout1>
            ...
            <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6">
                <div className="mb-6 flex items-center justify-between">
                    <div>
                        <h1 className="text-2xl font-extrabold text-white">Editar subasta</h1>
                        <p className="text-sm text-slate-500">Puja actual: {(auction.current_price_cents / 100).toFixed(2)} €</p>
                    </div>
                    <button
                        type="button"
                ...
                {auction.first_image ? (
                    <img src={auction.first_image} alt="" className="mb-5 h-40 w-full rounded-2xl object-cover" />
                ) : null}
                ...
                <form onSubmit={submit} className="space-y-5 rounded-2xl border border-white/10 bg-white/5 p-6">
                    ...
                    <Field label="Nuevas imágenes (reemplazan las actuales)" error={errors.images}>
                        <input
                            type="file"
                            accept="image/*"
                            multiple
